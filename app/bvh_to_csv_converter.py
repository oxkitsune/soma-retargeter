...
class Viewer:
    def __init__(self, viewer, config):
        ...
        self.retarget_target_options = ['unitree_g1', 'booster_k1']
        self.retarget_solver_options = ['Newton']
        self.retarget_solver_idx     = 0
        self.retarget_target_idx     = self.retarget_target_options.index(config.get('retarget_target', 'unitree_g1'))
        ...
        robot_type = config.get('retarget_target', 'unitree_g1')
        self.csv_config = csv_utils.get_csv_config(robot_type)
        ...
        if robot_type == 'unitree_g1':
            ...
            k1_mjcf = io_utils.get_package_root().parent / "booster-k1" / "k1.xml"
            robot_builder.add_mjcf(k1_mjcf)
            newton_utils.apply_mjcf_keyframe_qpos(robot_builder, k1_mjcf)
        else:
            raise ValueError(f"Unsupported retarget_target: {robot_type}")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a BVH file to load on startup.")
    parser.add_argument(
        "--csv",
        type=str,
        default=None,
        help="Path to a CSV file to load on startup.")
